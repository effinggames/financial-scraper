...
const EAFESpreadsheetURL = 'https://www.msci.com/webapp/indexperf/charts?indices=108,C,36&startDate=31%20Dec,%201969&endDate=24%20Jun,%202099&priceLevel=40&currency=15&frequency=M&scope=R&format=XLS&baseValue=false&site=gimi';
...
class EAFEScraper {
     ...
     */
    fetch() {
        Logger.info('Fetching EAFE index data');
        return Request.get(EAFESpreadsheetURL, { encoding: null }).then(xlsBuffer => {
            ...
            return Knex('europe.eafe_monthly').truncate().then(function() {
                Logger.info('Truncating old EAFE data');
                return Knex('europe.eafe_monthly').insert(formatData(valueData));
            }).then(function() {
