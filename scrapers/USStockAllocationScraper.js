...
class USStockAllocationScraper {
    ...
    fetch() {
        ...
        return Request.get(fredGraphUrl, { encoding: null }).then(xlsBuffer => {
            ...
            Assert.strictEqual(worksheet.B11.w, 'NCBEILQ027S_BCNSDODNS_CMDEBT_FGSDODNS_SLGSDODNS_FBCELLQ027S_DODFFSWCMI');
            ...
            const stockAllocationData = SpreadsheetHelper.columnRange('B', 12, lastRow).map(i => worksheet[i].v || null);
            ...
            return Knex('usa.stock_asset_allocation').truncate().then(function() {
